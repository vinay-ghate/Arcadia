document.addEventListener('DOMContentLoaded', () => {
    ...
    const PIECES = [
        ...
        { shape: [[1, 1], [1, 1]], className: 'block-o' },
        { shape: [[0, 1, 0], [1, 1, 1]], className: 'block-t' }, // Purple in your CSS
        { shape: [[0, 1, 1], [1, 1, 0]], className: 'block-s' },
        { shape: [[1, 1, 0], [0, 1, 1]], className: 'block-z' }, // Red in your CSS
        { shape: [[1, 0, 0], [1, 1, 1]], className: 'block-j' },
        { shape: [[0, 0, 1], [1, 1, 1]], className: 'block-l' }
    ];
    ...
    function move(dx, dy) {
        ...
        } else if (dy > 0) { // If move failed while going down, lock it
            lockPiece();
            clearLines();
            spawnNewPiece();
        }
    ...
    document.addEventListener('keydown', e => {
        ...
        if (e.key === 'ArrowDown') { dropCounter = dropInterval; } // Drop faster
        if (e.key === 'ArrowUp') rotate();
    });
    
    leftBtn.addEventListener('click', () => !isPaused && move(-1, 0));
    rightBtn.addEventListener('click', () => !isPaused && move(1, 0));
    downBtn.addEventListener('click', () => !isPaused && (dropCounter = dropInterval));
